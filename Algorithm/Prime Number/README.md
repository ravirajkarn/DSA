...
- Generator
  - [X] [Sieve of Eratosthenes](#sieve-of-eratosthenes) 
  - [ ] Sieve of Sundaram
  - [X] [Sieve of Atkin](#sieve-of-atkin)

- Checker 
  - [X] [Naive Approach (Trial Division)](#1-naive-approach-trial-division) 
  - [ ] [Sieve Methods](#2-sieve-methods-for-checking-multiple-numbers-up-to-a-limit)
    - [ ] [Sieve of Eratosthenes](#a-sieve-of-eratosthenes)
    - [ ] [Sieve of Atkin](#b-sieve-of-atkin-more-advanced)
  - [ ] [Probabilistic Primality Tests](#3-probabilistic-primality-tests-for-very-large-numbers)
    - [ ] [Miller-Rabin Primality Test](#a-miller-rabin-primality-test) 
  - [ ] [Deterministic Primality Tests](#4-deterministic-primality-tests-for-very-large-numbers)
  - [ ] [AKS Primality Test](#a-aks-primality-test)
...
**Pros:** Simple to understand and implement. Efficient enough for small to moderately sized numbers.
**Cons:** Can be slow for very large numbers.
...
3.  Start with `p = 2`.
4.  While $p^2 \le \text{limit}$:
    a.  If `is_prime[p]` is `True`, then `p` is a prime number. Mark all multiples of `p` (starting from $p^2$) as `False` (e.g., $p^2$, $p^2+p$, $p^2+2p$, etc.).
...
**Time Complexity (for generating all primes up to N):** $O(N \log \log N)$
**Space Complexity:** $O(N)$
**Pros:** Extremely efficient for finding *all* primes up to a limit. Very fast lookup for primality of numbers within the pre-calculated range.
